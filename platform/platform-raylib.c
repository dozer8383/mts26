...
void onInit(void) {
    ...
    UnloadImage(image);
    image = LoadImage("assets/large/tree-money.png");
...
void onEndFrame(int state, int year, int money, bool harvest, int screenWidth, int screenHeight, float frameTime) {
    ...
    if (state == 0) {
        ...
        DrawTextureEx(gameLogo,(Vector2){screenWidth/2+parallaxX-gameLogo.width*0.7*0.5,screenHeight/2+parallaxY-gameLogo.height*0.7*0.5},0,0.7,WHITE);
    }
    char printBuffer[16];
